"""
$ python3 solution18_2.py input18_test.txt 6 12 24
6,1

$ python3 solution18_2.py input18.txt 70 1024 3450
25,6
"""
...
# the size of the grid and the number of min/max number of corrupted blocks
# to read from the input file are specified as command-line arguments
size = int(sys.argv[2])
min_corrupted = int(sys.argv[3])
max_corrupted = int(sys.argv[4])

source = complex(0, 0)
target = complex(size, size)

# instead of increasing the number of corrupted blocks one by one, a binary
# search is performed to locate the exact number of corrupted blocks for
# which a path between the source and the target ceases to exist:
while min_corrupted <= max_corrupted:
    nb_corrupted = (min_corrupted + max_corrupted) // 2
    ...
    corrupted = list(read_input(lines=nb_corrupted))
    # remove the corrupted blocks from the map
    map.difference_update(corrupted)

    scores = search(map, source, target)
    # print_map()
    # print(f"{min_corrupted}-{max_corrupted} -> {nb_corrupted=}")
    if scores[source] < inf:
        # the path is reachable from the source: increase the minimum number
        # of corrupted blocks
        min_corrupted = nb_corrupted + 1
    else:
        # the path is not reachable from the source: decrease the maximum
        # number of corrupted blocks
        max_corrupted = nb_corrupted - 1
        # store the last corrupted block that fell: this might be the result
        last_corrupted = corrupted[-1]

print(f"{int(last_corrupted.imag)},{int(last_corrupted.real)}")